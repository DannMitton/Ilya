...
<footer class="page-footer">
	...
	<div class="footer-content">
		...
		<div class="url-cell">
			<a href="https://www.dannmitton.com" target="_blank" rel="noopener">www.dannmitton.com</a>
		</div>
...
<style>
	...
	/* ── Three-column footer: invisible table layout ───────── */

	.footer-content {
		display: grid;
		grid-template-columns: minmax(0, 1fr) auto auto;
		align-items: stretch; /* Key: stretch all cells to same height */
		gap: 0 32px; /* Slightly wider gap for visual breathing room */
	}

	/* Column 1: Attribution - fully justified, width expands to force 3-line French */
	.attribution-cell {
		text-align: justify;
		hyphens: auto; /* Allow hyphenation for better line breaks */
		padding-right: 8px; /* Micro-padding to prevent collision with URL column */
	}
	...
	.attribution-text {
		...
		font-family: var(--font-sans);
		font-size: 14px;
		line-height: 1.35; /* Tighter for compact 3-line fit */
		color: var(--ink-secondary);
		/* CRITICAL FIX: Remove all-small-caps to match visual size */
		text-transform: uppercase;
		letter-spacing: 0.08em; /* Slightly wider for uppercase readability */
		font-weight: 400;
	}
	...
	.attribution-text :global(em) {
		font-style: italic;
		text-transform: none; /* Preserve case for italicized title */
	...
	.attribution-text :global(a) {
		...
		text-transform: none; /* Preserve case for "CC BY-SA" */
		letter-spacing: normal;
	}
	...
	/* Column 2: URL - centred, bottom-aligned */
	.url-cell {
		display: flex;
		align-items: flex-end; /* Anchor to bottom */
		justify-content: center;
		white-space: nowrap;
		padding-bottom: 1px; /* Optical alignment with attribution baseline */
	}

	.url-cell a {
		font-family: var(--font-sans);
		font-size: 14px;
		text-transform: uppercase; /* Match pagination exactly */
	...
	/* Column 3: Pagination - flush right, bottom-aligned */
	.pagination-cell {
		display: flex;
		align-items: flex-end; /* Anchor to bottom */
		justify-content: flex-end;
	...
	.page-number {
		font-family: var(--font-sans);
		font-size: 14px;
		text-transform: uppercase; /* Explicitly uppercase, no variant needed */
		letter-spacing: 0.08em;
		color: var(--ink-secondary);
		font-weight: 400;
	}
