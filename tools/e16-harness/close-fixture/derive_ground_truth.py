#!/usr/bin/env python3
"""Mechanically derive harness-schema ground truth from close-fixture.musicxml.

Walks the MusicXML source directly. Never consults the rendered image.
Conventions (matched field-for-field against the harness corpus's
mussorgsky---sunless-01 ground-truth.json):
...
SRC = "/root/fixture/close-fixture.musicxml"
OUT = "/root/fixture/close-fixture-ground-truth.json"
...
ground_truth = {
    "pieceId": "e16-close-fixture---musescore3-mscore-font-01",
    "sourceMusxPath": "close-fixture.musicxml (MusicXML source, this procurement; no .musx exists)",
...
n_notes = sum(1 for n in notes if n["type"] == "note")
n_rests = sum(1 for n in notes if n["type"] == "rest")
print(f"wrote {OUT}: {len(measure_durations)} measures, {n_notes} notes, {n_rests} rests")
for n in notes:
    print(n["id"], n["type"], n.get("midi", "-"),
          f"{n['onset']['numerator']}/{n['onset']['denominator']}",
          f"dur {n['duration']['numerator']}/{n['duration']['denominator']}",
          f"abs {n['onsetAbsolute']}")
