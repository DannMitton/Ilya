#!/usr/bin/env python3
"""Mechanically derive harness-schema ground truth from fixture-a.musicxml.

Walks the MusicXML source directly. Never consults the rendered image.
Conventions (matched field-for-field against the close-fixture precedent,
claude/fable-memo-e16-close-fixture-procurement_2026-07-25.md, itself matched
against the harness corpus's mussorgsky---sunless-01 ground-truth.json):
...
printedGrouping, the literal text of any <direction><words> found at that
measure's start (None if absent). This is provenance for the numeral-with-
persistence capability the fixture exists to exercise; it is not consulted
by this script to compute anything else, and the measure-sum assertion
...
SRC = "fixture-a.musicxml"
OUT = "fixture-a-ground-truth.json"
...
for m_index, measure in enumerate(part.findall("measure")):
    ...
    # printed grouping numeral, if any, from a <direction><words> at this measure
    printed_grouping = None
    direction = measure.find("direction")
    if direction is not None:
        words = direction.find("direction-type/words")
        if words is not None:
            printed_grouping = words.text
    grouping_by_measure.append({"index": m_index, "printedGrouping": printed_grouping})
...
ground_truth = {
    "pieceId": "e16-irregular-fixture-a---numerals-with-persistence",
    "sourceMusxPath": "fixture-a.musicxml (MusicXML source, this procurement; no .musx exists)",
    ...
    "printedGroupingByMeasure": grouping_by_measure,
