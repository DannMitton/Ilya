# Brief for Code, r1 DRAFT: N.168's first slice, the held-note and turning comments, with N.172's intake
...
**Scope, DESK DEFAULT:** comments 1 (the held note at the top) and 2 (the open vowel turning at the secondo). Comment 3 (the [u]) waits on Fable's two open questions (r1 §5), which Dann has not answered.
...
7. **Strings.** Every sentence is an i18n entry with placeholders (`{vowel}`, `{pitch}`, `{seconds}`, `{semitones}`, `{bars}`). Pitches go through `pitchLabel` (`note-picker.ts:94`), as the rest of Insights does. English from r2; French only as Dann ruled it. **No French that he has not ruled goes into `i18n.ts`** (CONTRACT §6).
8. **Citations.** One registry of sources: citekey, author, short title, full reference, and, for each cited row, the page, the section heading, and a short quotation. The visible citation reads author, short title, and page. The tap reads the rest. **Section headings are missing for most extraction rows.** Where one is missing, print no heading. Do not invent one.
