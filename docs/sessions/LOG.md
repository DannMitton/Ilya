...
### The "Closed and parked" tracker table, moved from STATE.md at the close of 2026-09-10
...
## Block 10. Moved from STATE.md at the close of 2026-09-10 late

The "THE ONE THING, at the close of 2026-09-10 06:15" block, verbatim, superseded by the 23:55 block after the walk and increment 2 (`8032489`).
...
### The blocking-set table (N.67, N.72, N.58, N.59), verbatim, moved the same close
...
## Block 13. Moved from STATE.md at the close of 2026-09-13, the colour session

**Stages 3a and 3b, the close of stage 2, and the desk's two errors along the
way.** All of it closed and committed: `45f7cd4`, `6c53a9d`, `2a39366`,
`a3e2de4`, `28b48fa`, `6a24169`, pushed and deployed
(`dpl_6jRMMzsA9uDDYiUcsRaU8QNFYVKX`, READY, walked by Dann at 1400 px).

**Three things were lifted out of this block before it moved, because they are
live**, and they sit in `STATE.md` §THE ONE THING: the three tokens Code could
not get on screen, Code's caveat that rose and lavender carry fallback hexes
that would mask a broken name, and the fact that stage 4 is already fully
specified.
...
## Block 14. Moved from STATE.md at the close of 2026-09-14

**Verbatim, nothing reworded.** This is the whole of `STATE.md` §THE ONE THING
as it stood before the close of 2026-09-14: the stage 3a/3b account, the four
findings of the 2026-09-13 walk, the twenty values, the prune-session close,
the four-ships close of 2026-09-12 into 2026-09-13, N.121's unbuilt half, and
the two tripwire narratives.

**Four things were live and were rewritten fresh in `STATE.md` rather than
moved:** the stage 4 spec and its walk, the twenty values, INSIGHTS WHAT IS
OPEN, and BRIEFS WRITTEN AND NOT RUN. **N.121's unbuilt half closed on
2026-09-14** when N.134 shipped, and the 2026-09-13 walk's finding 1 closed
the same night; both accounts are in `STATE.md` §THE ONE THING.
...
## Block 16. N.138 CLOSED 2026-09-14. The loupe supplies the meter.

**All three increments shipped and walked by Dann.** Increment 1 as `78f3db8`,
increments 2 and 3 as `8bb406c`, the latter served by
`dpl_NVPuRWJzticKSxLs9bLp6RMkaLpw` on the branch alias, sha checked before he was
sent to it. **His words on the walk of `8bb406c`: increment 2 "Beautiful!
confirmed", increment 3 "Yes this is ideal".**

Gate 4 moved 1144 to 1150 to **1163**; gate 5 moved 547 to **548** for the new
`data-key-signature` marker test. The desk moved `~/Downloads/ilya-ship.sh:79`
and `:80` both times, per §THE DESK MOVES THE GATE LINE, keeping
`ilya-ship.sh.bak-1144` and `ilya-ship.sh.bak-1150-547`.

**THREE THINGS THAT CAME OUT OF THIS ITEM AND ARE STILL LIVE. They are NOT closed
by it.**

1. **The selection ring can land under the system's opaque ground**, on the page
   and in the loupe. Pre-existing, found by Code, deliberately not fixed. Caught
   in `INBOX.md`; details in `memo-n138-loupe-meter_r1_2026-09-14.md` §7.7.
   **NOT ESTABLISHED as the cause of the missing box on Sunless m. 18**, because
   the pane was hidden when that was observed.
2. **Whether a tap ever picked the wrong note** before increment 3 scoped taps to
   the body's own targets. The clef-and-key crop carried a full copy of the
   system with live tap targets, and that predates N.138. NOT ESTABLISHED.
3. **N.140**, the loupe's stave-space floor, was numbered out of this work and is
   open in `OPEN.md`.

**Also settled here, and it does not need revisiting.** Finding 3 of the
2026-09-13 walk, "the measures are half the rhythmic value they should be", is
CLOSED and was never a defect: T05 declares 2/4 once at measure 1 and never
changes it, its Bass measures fill 2/4 exactly, and Dann checked the printed
score on 2026-09-14 and it is 2/4. Finding 2 of that walk became N.139 and is
open.

**Whether m. 17's stave space is readable is SETTLED.** Dann was shown all five
scales drawn at true CSS pixel size in
`drawing-loupe-stave-scales_r1_2026-09-14.html` and said they all read well. The
worst is now 3.93 px, from 3.96, after increment 3 ran the measure up to its
barline. He was told before the walk.
...
## Block 20. Moved from STATE.md and OPEN.md at the close of 2026-09-17 (early)

N.143, N.142 with the loupe French, N.144 and N.145 all shipped and were walked by Dann on 2026-09-16. N.126, N.138 and N.133 tracker marks moved with them (their accounts were already in blocks 16 and 17). Moved verbatim.

### From STATE.md §THE ONE THING: the 2026-09-16 read-first block and its walk narrative
...
> **2026-09-16 late: pass 02 planned,** `~/Documents/Voice Pedagogy Library/Insights Research/plan-pass02_r1_2026-09-16.md`. **Elsevier answered:** Journal of Voice at 120 USD, print and online, back issues from 1987; Dann asked how to buy. Usage at 20:00 the same night: all models 40%, Fable 15%.
>
...
- `[x]` **N.145 DONE 2026-09-16 for the drop, shipped `7c596f7`, walked by Dann: *"Instantaneous! No button click needed! :))"*** Fresh incognito, T05 dropped: the input field filled, Transcription drew in full, Score markup drew with `146 / 146 placed`, Insights drew its not-calibrated page, the conversion notice stood with its **Dismiss**, and no Transcribe and fit pill. **Not walked: a poem pasted after a score with no words** (no wordless score was to hand). ~~A dropped score arrives at once; no Continue to analysis. RULED BY DANN 2026-09-16: option 2, instant for every file. EXTENDED the same night: Transcribe and fit is removed, and a poem arriving after a wordless score is placed at once.** Brief r2 supersedes r1.~~ Numbered 2026-09-16, THE NUMBER IS A DESK DEFAULT. Spec `OPEN.md` §N.145, brief `../sessions/brief-n145-instant-score-arrival_r2_2026-09-16.md` (r1 superseded).
...
## Block 25. N.148, N.149 and N.150 CLOSED 2026-09-20. Moved from `OPEN.md` at the close.

**Verbatim from `OPEN.md`, nothing reworded.** Shipped over `6ca97db` (the two modes and
the segmented pill), `be792b6` (the pill's fill marks an open panel), `38dac87` (the
carets follow the panel) and `c582892` (Corrections leaves the drawer, the band becomes
Voice, the corrected-count sentence goes). **Every one walked by Dann the same day, on
desk-driven screenshots in his own Chrome.** « Voix » ratified 2026-09-20.

**The Syllables-mode spacing subsection that sat inside this section did NOT come here.**
It moved to N.153's section in `OPEN.md`, being loupe-local spacing rather than the mode
split, and it is still open.
...
## Block 26. Three superseded closes, moved from `STATE.md` at the close of 2026-09-20.

**Verbatim, nothing reworded.** In file order: the close of 2026-09-19 into 2026-09-20
(00:20), then the close of 2026-09-19 (22:00), then the close of 2026-09-18 (04:00).
**Each names a floor and a one thing that the close of 2026-09-20 supersedes:** the
floor is now `c582892` and N.148, N.149 and N.150 are closed. **The walk of 2026-09-19
into 2026-09-20 did NOT move**; it stays in `STATE.md` because it still holds live
items that the mode split did not answer.
...
## Block 27. N.136, N.158 and N.159 CLOSED 2026-09-21. Moved from `OPEN.md` at the close

**All three closed in one ship, `1d18514`, walked by Dann on his own library in his Chrome on
the branch alias at about 03:45.** N.159 built the drawing step; N.136 and N.158 were the two
switches that had no route to the score, so they closed with it.

**Measured on Dann's own library at the walk, not on the fixture:** Reconstitution changes 16
notes, Open syllables changes 24 notes and moves no Cyrillic, and the seat instrument reads
**67 drawn live, 29 kept as stored, of 96 seated**.

**Dann's account of what reconstitution is was moved to `PRODUCT.md` §WHAT RECONSTITUTION IS
before this block was written**, so it is not buried here.

**The two sections below are verbatim from `OPEN.md`.**
...
## Block: 2026-09-25 about 15:05. STATE.md's THE ONE THING block of 13:10, moved verbatim. Its rulings live in PRODUCT.md, OPEN.md §N.164 and §N.173, the templates file, and the two N.172 drafts
